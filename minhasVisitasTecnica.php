...
$sql = "SELECT * FROM minhasvt mv INNER JOIN visitastecnicas vt ON mv.fkVisitasTecnicas = vt.idVt INNER JOIN usuarios u ON mv.fkUsuario = u.idUsuario WHERE mv.fkUsuario = '$id'";
...
if (isset($_GET['deletar'])) {
    //pega o total de inscritos
    $id = $_GET['deletar'];
    $sqlDelete = mysqli_query($conn, "UPDATE visitastecnicas SET status = '0' WHERE visitastecnicas.idVt = '$id';");
    if ($sqlDelete) {
        $_SESSION['msn2'] = '<div class="alert alert-success alert-dismissible fade show text-center" role="alert">
        <strong>Exclusão logica</strong> aplicada!
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
    } else {
        $_SESSION['msn2'] = '<div class="alert alert-warning alert-dismissible fade show text-center" role="alert">
        Ops! Algo deu errado.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
    }
...
if (isset($_GET['publicar'])) {
    //pega o total de inscritos
    $id = $_GET['publicar'];
    $sqlDelete = mysqli_query($conn, "UPDATE visitastecnicas SET status = '1' WHERE visitastecnicas.idVt = '$id';");
    if ($sqlDelete) {
        $_SESSION['msn2'] = '<div class="alert alert-success alert-dismissible fade show text-center" role="alert">
        Visita técnica <strong>publicada</strong>!
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
    } else {
        $_SESSION['msn2'] = '<div class="alert alert-warning alert-dismissible fade show text-center" role="alert">
...
if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM usuarios WHERE idUsuario = '$id'")) > 0) {
    $nivelUsuario = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM usuarios WHERE idUsuario = '$id'"));
    if ($nivelUsuario['fkNivelUsuario'] >= 1 && $nivelUsuario['fkNivelUsuario'] <= 2) { ?>

<h1 class="h1 text-center my-3">Gerenciar Visitas</h1>
<?php

        $exeSql2 = mysqli_query($conn, "SELECT * FROM visitastecnicas WHERE fkResponsavel = '$id'");

        if (mysqli_num_rows($exeSql2) > 0) { ?>
<table id="example2" class="table table-striped table-bordered table-hover my-3">
    <thead class="text-center">
        <th>status</th>
        <th>Nome / Local</th>
        <th>Data</th>
        <th>Vagas / Nº inscritos</th>
        <th>Ações</th>
    ...
    <tbody>
        <?php
                    while ($dd = mysqli_fetch_assoc($exeSql2)) {
                    ?>
        ...
        <tr>
            <td class="text-center">
                <?php
                                $dd['status'] == 1 ? $status =  ' 
                                <a href="home.php?pages=minhasVisitasTecnica.php&deletar=' . $dd['idVt'] . '"
                            class="btn btn-outline-danger w-100" onclick="return confirm("Confirma a exclusão da vissita?")"
                            data-bs-toggle="tooltip" data-bs-placement="bottom" title="Remover inscrião da visita"><i class="bi bi-eye-slash-fill"></i> Excluir! </i>
                            </a>' : $status =  ' 
                                    <a href="home.php?pages=minhasVisitasTecnica.php&publicar=' . $dd['idVt'] . '"
                            class="btn btn-outline-success w-100" onclick="return confirm("Confirma a exclusão da vissita?")"
                            data-bs-toggle="tooltip" data-bs-placement="bottom" title="Remover inscrião da visita"><i class="bi bi-eye-fill"></i> publicar! </i>
                            </a>';
                                echo $status; ?>
            ...
            <td><?php echo $dd['nomeVt'] ?> / <?php echo $dd['cidadeVt'] . ' - ' . $dd['ufVt'] ?></td>
            <td class="text-center"><?php echo date('d/m/Y', strtotime($dd['dataVt'])) ?></td>
            ...
            <td class="text-center">
                ...
                <div class="row">
                    <div class="col-5">
                        <a href="home.php?pages=minhasVisitasTecnica.php&editar=<?php echo $dd['idVt'] ?>"
                            class="btn btn-warning w-100"> <i class="bi bi-pencil-square"></i>
                            editar </a>
                    </div>
                    <div class="col-7">
                        <a href="home.php?pages=minhasVisitasTecnica.php&frequencia=<?php echo $dd['idVt'] ?>"
                            class="btn btn-warning w-100"><i class="bi bi-clipboard-check"></i>
                            Frequência </a>
